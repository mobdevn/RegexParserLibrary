/**
 * # Regex Utility Library (Analyzer Fix)
 *
...
class RegexAnalyzer(private val components: List<RegexComponent>) {
    ...
    fun calculateMaxLength(): Int {
        ...
        return if (lookaheadMax != null) minOf(calculatedLength, lookaheadMax) else calculatedLength
    }
    ...
    private fun calculateBranchMaxLength(branch: List<RegexComponent>): Int = branch.sumOf { component ->
        when (component) {
            ...
            is RegexComponent.Group -> {
                if (component.quantifier?.max == Int.MAX_VALUE) return RegexConstants.MAX_SAFE_LENGTH
                val groupLength = calculateBranchMaxLength(component.components)
                val quantifierMax = component.quantifier?.max ?: 1
    ...
    private fun findLiteralsRecursive(
        ...
        for (component in branch) {
            when (component) {
                ...
                is RegexComponent.Alternation -> currentPosition += calculateBranchMaxLength(listOf(component))
                is RegexComponent.Anchor, is RegexComponent.Lookahead -> { /* Do nothing, zero-width */ }
                else -> currentPosition += calculateBranchMaxLength(listOf(component))
            }
        }
