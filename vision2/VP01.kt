...
class RegexAnalyser(val pattern: String) {
    ...
    fun isValid(input: String): Boolean {
        return try { this.pattern.toRegex().matches(input) } catch (e: Exception) { false }
    }

    fun extractCapturedGroups(input: String): List<String>? {
        return try {
            this.pattern.toRegex().matchEntire(input)?.groupValues?.drop(Constants.CAPTURED_GROUP_OFFSET)
    ...
    fun formatInput(userInput: String): FormattingResult? {
        ...
        // In formatting, we use the pattern without the lookahead for simplicity
        val patternToFormat = RegexPatternParser.extractLookaheadLengthConstraint(this.pattern).second

        while (patternCursor < patternToFormat.length) {
            val char = patternToFormat[patternCursor]
            ...
            when (char) {
                Constants.Chars.GROUP_START -> {
                    val end = RegexStringUtils.findClosingParen(patternToFormat, patternCursor)
                    elementBaseLength = RegexAnalyser(patternToFormat.substring(patternCursor + 1, end)).getMaxLength()
                    nextIndexAfterElement = end + 1
                }
                Constants.Chars.CLASS_START -> {
                    elementBaseLength = Constants.SINGLE_CHAR_LENGTH
                    nextIndexAfterElement = RegexStringUtils.findClosingBracket(patternToFormat, patternCursor) + 1
                }
            ...
            val quantifierResult = RegexPatternParser.parseAndApplyQuantifier(patternToFormat, nextIndexAfterElement, elementBaseLength)
        ...
        return if (inputCursor <= cleanInput.length) { // Allow unused input
            FormattingResult(
    ...
    private object RegexPatternParser {
        ...
        /**
         * NEW: A helper regex to find and extract constraints from lookaheads.
         * It looks for patterns like `(?=^.{min,max}$)`
         */
        private val lookaheadConstraintRegex = "\\(\\?=\\^\\.\\{(?:([0-9]+))?,(?:([0-9]+))?\\}\\$\\)".toRegex()

        internal fun extractLookaheadLengthConstraint(pattern: String): Pair<Int?, String> {
            val match = lookaheadConstraintRegex.find(pattern)
            return if (match != null) {
                val max = match.groupValues.getOrNull(2)?.toIntOrNull() // Group 2 is the max value
                val remainingPattern = pattern.replace(match.value, "")
                Pair(max, remainingPattern)
            } else {
                Pair(null, pattern)
            }
        }
        ...
        fun parse(pattern: String): AnalysisResult {
            // FIX: Extract lookahead constraint first.
            ...
            // FIX: Apply lookahead constraint to the final calculation.
            val finalMaxLength = when {
                // If the pattern could be infinite BUT a lookahead provides a max, use the lookahead's max.
                hasInfinite && lookaheadMax != null -> lookaheadMax
                // If it's infinite and there's no constraint, it's truly infinite.
                hasInfinite -> Constants.LENGTH_INFINITE
                // Otherwise, use the calculated max length.
                else -> globalMax
            }

            return AnalysisResult(
                maxLength = finalMaxLength,
                hasInfiniteLength = hasInfinite && lookaheadMax == null, // Only truly infinite if not constrained
        ...
        private fun handleGroupToken(pattern: String, index: Int, state: BranchState): Int {
            ...
            if (groupState.hasInfinite) { state.hasInfinite = true }
            // The max length of a repeating group is now handled by the top-level hasInfinite flag
        ...
        private fun applyQuantifierAndUpdateState(quantifierResult: QuantifierParseResult, literal: String?, literalPos: Int, state: BranchState) {
            // For repeating groups, hasInfinite flag is more important than adding length here.
            if (quantifierResult.isInfinite) {
